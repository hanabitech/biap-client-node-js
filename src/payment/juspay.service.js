...
class JuspayService {
  ...
  async signPayload(data) {
    try {
      ...
      if (payload) {
        ...
        const privateKeyHyperBeta = await readFile(
          process.env.JUSPAY_SECRET_KEY_PATH,
          "utf-8"
        );
  ...
  async verifyPayment(data) {
    try {
      ...
      if (
        dbResponse?.paymentStatus === null ||
        dbResponse?.paymentStatus !== status
      ) {
